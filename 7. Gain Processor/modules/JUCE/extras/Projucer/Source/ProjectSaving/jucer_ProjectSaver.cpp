...
{
    const auto pluginDefines = getGainProcessorDefines();
...
void ProjectSaver::writeAppConfig (MemoryOutputStream& out, const OwnedArray<LibraryModule>& modules, const String& userContent)
{
    ...
    if (getPluginDefinesFile().existsAsFile() && getGainProcessorDefines().isNotEmpty())
    ...
    auto isStandaloneApplication = (! type.isGainProcessor() && ! type.isDynamicLibrary());
...
String ProjectSaver::getGainProcessorDefines() const
{
    const auto flags = project.getGainProcessorFlags();
