...
public:
    ...
    void createExporterProperties (PropertyListBuilder& props) override
    {
        ...
        if (getProject().isGainProcessorProject())
...
protected:
    ...
    public:
        ...
        {
            if (project.isGainProcessorProject())
...
public:
    ...
    */
    ...
    struct XcodeTarget final : build_tools::ProjectType::Target
    {
        ...
        {
            if (! owner.project.isGainProcessorProject())
                return;
        ...
        for (auto* target : targets)
        {
            ...
            if (target->type != XcodeTarget::AggregateTarget)
            {
                ...
                if (target->type == XcodeTarget::SharedCodeTarget
                     || (! project.isGainProcessorProject()))
            ...
            if (project.isGainProcessorProject() && project.shouldBuildAUv3()
                && project.shouldBuildStandalonePlugin() && target->type == XcodeTarget::StandalonePlugIn)
                embedAppExtension();

            if (project.isGainProcessorProject() && project.shouldBuildUnityPlugin()
...
private:
    ...
    void addEntitlementsFile (XcodeTarget& target) const
    {
        ...
        options.isiOS                           = isiOS();
        options.isGainProcessorProject            = project.isGainProcessorProject();
    ...
    StringArray getProjectObjectAttributes() const
    {
        ...
        if (projectType.isGUIApplication() || projectType.isGainProcessor())
        {
