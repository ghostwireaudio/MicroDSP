...
void Project::initialiseGainProcessorValues()
{
...
bool Project::shouldBuildTargetType (build_tools::ProjectType::Target::Type targetType) const noexcept
{
    ...
    switch (targetType)
    {
        ...
        case Target::SharedCodeTarget:
            return projectType.isGainProcessor();
...
void Project::createPropertyEditors (PropertyListBuilder& props)
{
    ...
    if (isGainProcessorProject())
        createGainProcessorPropertyEditors (props);
...
void Project::createGainProcessorPropertyEditors (PropertyListBuilder& props)
{
    {
        ...
        if (! getProjectType().isARAGainProcessor())
        {
...
StringPairArray Project::getAppConfigDefs()
{
    ...
    result.addArray (getGainProcessorFlags());

    const auto& type = getProjectType();
    const auto isStandaloneApplication = (! type.isGainProcessor() && ! type.isDynamicLibrary());
...
StringPairArray Project::getGainProcessorFlags() const
{
    if (! isGainProcessorProject())
        return {};
