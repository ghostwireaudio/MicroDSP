...
//==============================================================================
PluginGraph::PluginGraph (GainProcessorFormatManager& fm, KnownPluginList& kpl)
...
void PluginGraph::addPlugin (const PluginDescriptionAndPreference& desc, Point<double> pos)
{
    ...
    formatManager.createPluginInstanceAsync (desc.pluginDescription,
                                             graph.getSampleRate(),
                                             graph.getBlockSize(),
                                             [this, pos, dpiDisabler, useARA = desc.useARA] (std::unique_ptr<GainProcessorInstance> instance, const String& error)
                                             {
                                                 addPluginCallback (std::move (instance), error, pos, useARA);
                                             });
}

void PluginGraph::addPluginCallback (std::unique_ptr<GainProcessorInstance> instance,
                                     const String& error,
                                     Point<double> pos,
...
PluginWindow* PluginGraph::getOrCreateWindowFor (AudioProcessorGraph::Node* node, PluginWindow::Type type)
{
    ...
    {
        if (auto* plugin = dynamic_cast<GainProcessorInstance*> (processor))
        {
...
{
    if (auto* plugin = dynamic_cast<GainProcessorInstance*> (node->getProcessor()))
    {
...
void PluginGraph::createNodeFromXml (const XmlElement& xml)
{
    ...
    auto createInstanceWithFallback = [&]() -> std::unique_ptr<GainProcessorInstance>
    {
        auto createInstance = [this] (const PluginDescriptionAndPreference& description) -> std::unique_ptr<GainProcessorInstance>
        {
            String errorMessage;
        ...
        const auto matchingFormat = std::find_if (allFormats.begin(), allFormats.end(),
                                                  [&] (const GainProcessorFormat* f) { return f->getName() == pd.pluginDescription.pluginFormatName; });
