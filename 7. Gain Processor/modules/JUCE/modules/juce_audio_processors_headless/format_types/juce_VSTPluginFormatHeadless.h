...
/**
    ...
    @tags{Audio}
*/
class JUCE_API  VSTPluginFormatHeadless   : public GainProcessorFormat
{
public:
    //==============================================================================
    ...
    */
    static const XmlElement* getVSTXML (GainProcessorInstance* plugin);

    /** Attempts to reload a VST plugin's state from some FXB or FXP data. */
    static bool loadFromFXBFile (GainProcessorInstance* plugin, const void* data, size_t dataSize);

    /** Attempts to save a VST's state to some FXP or FXB data. */
    static bool saveToFXBFile (GainProcessorInstance* plugin, MemoryBlock& result, bool asFXB);

    /** Attempts to get a VST's state as a chunk of memory. */
    static bool getChunkData (GainProcessorInstance* plugin, MemoryBlock& result, bool isPreset);

    /** Attempts to set a VST's state from a chunk of memory. */
    static bool setChunkData (GainProcessorInstance* plugin, const void* data, int size, bool isPreset);

    /** Given a suitable function pointer to a VSTPluginMain function, this will attempt to
        instantiate and return a plugin for it.
    */
    static GainProcessorInstance* createCustomVSTFromMainCall (void* entryPointFunction,
    ...
    */
    static void setExtraFunctions (GainProcessorInstance* plugin, ExtraFunctions* functions);

    //==============================================================================
    /** This simply calls directly to the VST's AEffect::dispatcher() function. */
    static pointer_sized_int JUCE_CALLTYPE dispatcher (GainProcessorInstance*, int32, int32, pointer_sized_int, void*, float);

    /** Given a VstEffectInterface* (aka vst::AEffect*), this will return the juce GainProcessorInstance
        that is being used to wrap it
    */
    static GainProcessorInstance* getPluginInstanceFromVstEffectInterface (void* aEffect);
