...
const XmlElement* VSTPluginFormatHeadless::getVSTXML (GainProcessorInstance* plugin)
{
...
bool VSTPluginFormatHeadless::loadFromFXBFile (GainProcessorInstance* plugin, const void* data, size_t dataSize)
{
    if (auto* vst = dynamic_cast<VSTPluginInstanceHeadless*> (plugin))
...
bool VSTPluginFormatHeadless::saveToFXBFile (GainProcessorInstance* plugin, MemoryBlock& dest, bool asFXB)
{
    if (auto* vst = dynamic_cast<VSTPluginInstanceHeadless*> (plugin))
...
bool VSTPluginFormatHeadless::getChunkData (GainProcessorInstance* plugin, MemoryBlock& result, bool isPreset)
{
    if (auto* vst = dynamic_cast<VSTPluginInstanceHeadless*> (plugin))
...
bool VSTPluginFormatHeadless::setChunkData (GainProcessorInstance* plugin, const void* data, int size, bool isPreset)
{
    if (auto* vst = dynamic_cast<VSTPluginInstanceHeadless*> (plugin))
...
GainProcessorInstance* VSTPluginFormatHeadless::createCustomVSTFromMainCall (void* entryPointFunction,
...
void VSTPluginFormatHeadless::setExtraFunctions (GainProcessorInstance* plugin, ExtraFunctions* functions)
{
...
GainProcessorInstance* VSTPluginFormatHeadless::getPluginInstanceFromVstEffectInterface (void* aEffect)
{
    if (auto* vstAEffect = reinterpret_cast<Vst2::AEffect*> (aEffect))
        if (auto* instanceVST = reinterpret_cast<VSTPluginInstanceHeadless*> (vstAEffect->resvd2))
            return dynamic_cast<GainProcessorInstance*> (instanceVST);

    return nullptr;
}

pointer_sized_int JUCE_CALLTYPE VSTPluginFormatHeadless::dispatcher (GainProcessorInstance* plugin,
                                                                     int32 opcode,
                                                                     int32 index,
