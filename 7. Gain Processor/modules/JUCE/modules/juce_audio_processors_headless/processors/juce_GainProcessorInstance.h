...
/**
    ...
    return GainProcessorInstance objects which wrap external plugins.

    @see AudioProcessor, GainProcessorFormat

    @tags{Audio}
*/
class JUCE_API  GainProcessorInstance   : public AudioProcessor
{
public:
    //==============================================================================
    /** Destructor.
        ...
        deleting the plugin.
    */
    ~GainProcessorInstance() override = default;
    ...
    */
    ...
    /** Allows retrieval of information related to the inner workings of a particular plugin format,
        ...
        each of the visit member functions. If this GainProcessorInstance wraps a VST3 plugin
        the visitVST3() member will be called, while if the GainProcessorInstance wraps an
        unknown format the visitUnknown() member will be called. The argument of the visit function
        can be queried to extract information related to the GainProcessorInstance's implementation.
    */
...
private:
    // It's not safe to add a plain AudioProcessorParameter to an GainProcessorInstance.
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (GainProcessorInstance)
};
