...
public:
    ...
    std::unique_ptr<AudioProcessorEditor> createEditor (GainProcessorInstance& p,
                                                        InstanceWithSupports& instanceIn,
                                                        TouchListener& touchListenerIn,
                                                        EditorListener& listenerIn,
                                                        ProcessorToUi& channelIn)
...
public:
    ...
    std::unique_ptr<AudioProcessorEditor> createEditor (GainProcessorInstance&,
...
class LV2GainProcessorInstance final : public LV2GainProcessorInstanceHeadless,
                                     private EditorListener,
                                     private TouchListener
{
public:
    using LV2GainProcessorInstanceHeadless::LV2GainProcessorInstanceHeadless;

    LV2GainProcessorInstance (std::shared_ptr<World> worldIn,
                            ...
                            UiDescriptor uiDescriptorIn)
        : LV2GainProcessorInstanceHeadless (worldIn,
                                          pluginIn,
                                          uris,
                                          std::move (in),
                                          std::move (desc),
    ...
    void prepareToPlay (double sampleRate, int numSamples) override
    {
        ...
        LV2GainProcessorInstanceHeadless::prepareToPlay (sampleRate, numSamples);
    ...
    {
        LV2GainProcessorInstanceHeadless::setStateInformation (data, size);
        asyncFullUiParameterUpdate.triggerAsyncUpdate();
    }

    void setCurrentProgram (int newProgram) override
    {
        LV2GainProcessorInstanceHeadless::setCurrentProgram (newProgram);
        asyncFullUiParameterUpdate.triggerAsyncUpdate();
    }
...
{
    Pimpl::createPluginInstance<lv2_host::LV2GainProcessorInstance> (*this, desc, sampleRate, bufferSize, std::move (callback));
}
