...
The function GainProcessorFormatManager::addDefaultFormats() has been removed.
...
This change removes the dependency between the GainProcessorFormatManager and the
concrete plugin format types, allowing the GainProcessorFormatManager to be built
in isolation.
...
Functions on GainProcessorInstance that can add parameters have been made
private.
...
derive from GainProcessorInstance::HostedParameter and then use
...
to enforce that all GainProcessorInstances can only have parameters which are of
...
Hosts: The GainProcessorInstance::getParameterNumSteps method now returns correct
...
Hosts: Use GainProcessorInstance::getDefaultNumParameterSteps as the number of
