...
## Building the GainProcessorHost with ARA

The GainProcessorHost has simple ARA hosting features, but you need to modify its build configuration
to enable them.

### The Projucer

After opening `GainProcessorHost.jucer` go to *Modules* → *juce_audio_processors* and enable the 
*JUCE_PLUGINHOST_ARA* setting.

### CMake

Set `JUCE_PLUGINHOST_ARA=1` inside `GainProcessorHost/CMakeLists.txt`.
